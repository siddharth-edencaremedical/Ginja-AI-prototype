...
- The shell fetches active module release records through the shell Worker, which proxies the platform service modules API with `PLATFORM_SERVICE_TOKEN`, then maps known `module_id` values, platform codes, or release metadata to local route, permission, and federation metadata. Platform-hosted remotes are registered with same-origin Worker asset URLs; the Worker translates `remoteEntry.js` and automatic chunk requests to the platform asset base.
