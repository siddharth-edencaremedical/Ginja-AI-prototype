...
pnpm dev:product-config    # Product Config remote — http://localhost:4201
pnpm dev:underwriting      # Underwriting remote — http://localhost:4202
...
apps/
  ...
  product-config/   Vertical remote (port 4201) — product/catalog domain
  underwriting/     Vertical remote (port 4202) — underwriting domain
packages/
...
Cross-vertical imports (`scope:product-config` ↔ `scope:underwriting`) are forbidden. Platform packages cannot depend on apps. These rules are encoded in `eslint.config.mjs` and `nx.json` — `pnpm lint` enforces them.
